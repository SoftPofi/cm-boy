...
class CmAlgo:
    ...
    def __init__(self, config, confidential_config, args):
    ...
    def adjust_price(self, card, listing, listing_orig):
        if not self.is_position_in_range(card, listing):
            old_card_price = card["price"]
            if self.match_price_of_target_offer(card, listing, listing_orig):
    ...
    def is_position_in_range(self, card, listings):
        my_position = self.get_position_in_list(card, listings)
        if self.get_min_pos_of_card(card) <= my_position <= self.get_max_pos_of_card(card):
            return True
        else:
            return False

    def is_price_of_card_already_min(self, card):
        if card["price"] <= self.get_min_price_for_card(card):
            return True
        else:
            return False

    def get_min_pos_of_card(self, card):
    ...
    def is_entry_my_own(self, listed_card):
    ...
    def get_min_price_for_card(self, card):
        min_price_parameter = self.card_type_parameter(card)
        return self.config["algo_parameter"][card["product"]["rarity"]]["min_price"][min_price_parameter]

    def card_type_parameter(self, card):
    ...
    def match_price_of_target_offer(self, card, listing, listing_orig):
        within_bound, target_position = self.verify_listing_boundaries(card, listing, self.get_target_pos_for_card(card))
        if not within_bound:
            return False
        if listing["article"][target_position]["price"] <= self.get_min_price_for_card(card):
            if self.is_price_of_card_already_min(card):
                return False
            card["price"] = self.get_min_price_for_card(card)
            return True
        else:
    ...
    def verify_listing_boundaries(self, card, listing, target_position):
    ...
    def parameter_for_card(self, card):
        ret_dict = {}
        ret_dict.update(
